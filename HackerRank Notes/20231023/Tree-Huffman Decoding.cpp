...
void decode_huff(node * root, string s) {
    node *current = root;  // Initialize current node as root
    string result = "";  // To store the final decoded string

    // Iterate through each character in the encoded string
    for (int i = 0; i < s.length(); i++) {
        // Go to left child if the character is '0', and right child otherwise
        current = (s[i] == '0') ? current->left : current->right;

        // If this node is a leaf node, it contains one of the input characters
        if (current->left == nullptr && current->right == nullptr) {
            result += current->data;  // Add the character to the result string
            current = root;  // Reset current node to root
        }
    }

    cout << result << endl;  // Print the decoded string
}
